...
If it is not possible for you to use a C#7-compatible compiler, we have added a compatibility wrapper around `Arg` called `CompatArg`, which has all the same members as `Arg` just without the `ref` return type. If you replace `Arg.` references with `CompatArg.` in your project then you can continue to use the old compiler with NSubstitute 4.x. You can create your own `ArgC` or similar wrapper that delegates to `CompatArg` within your project if you want a shorter name within your code. We've used the longer but more descriptive `CompatArg` to try to avoid confusion with people seeing `Arg.` and `ArgC.` in Intellisense/autocomplete.
